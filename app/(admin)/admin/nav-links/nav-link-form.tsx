...
import type { CreateNavLinkInput } from "@/lib/cms/types";
...
type Props = { defaultValues?: CreateNavLinkInput & { id: string } };

const NAV_LINK_GROUPS = ["HEADER", "FOOTER_NAV", "FOOTER_SERVICE"] as const;
...
export function NavLinkForm({ defaultValues }: Props) {
  const router = useRouter();
  const isEdit = !!defaultValues?.id;
  const endpoint = isEdit
    ? `/cms/nav-links/${defaultValues.id}`
    : "/cms/nav-links";

  const form = useForm<CreateNavLinkInput>({
    ...
    defaultValues: (defaultValues ?? {
      ...
      sortOrder: 0,
      published: true,
    }) as CreateNavLinkInput,
  });

  async function onSubmit(data: CreateNavLinkInput) {
    try {
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-slate-900">
        {isEdit ? "Edit Nav Link" : "New Nav Link"}
      </h1>
            ...
            render={({ field }) => (
              <FormItem>
                <div className="flex items-center gap-2">
                  ...
                  />
                  <FormLabel>Has Dropdown</FormLabel>
                </div>
                <FormMessage />
              </FormItem>
            )}
