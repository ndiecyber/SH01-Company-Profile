...
export default function ProjectsListPage() {
    ...
    useEffect(() => {
        async function load() {
            const { data } = await api.get<Project[]>("/cms/projects");
            setItems(data);
            setLoading(false);
        }
        load();
    ...
    async function handleDelete(id: string) {
        try {
            await apiDelete("/cms/projects/" + id);
            setRefresh((n) => n + 1);
        } catch (error) {
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/admin/projects/new" className="inline-flex items-center gap-1.5 rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand/90">
                            ...
                            <tr key={item.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-2">
                                        <Link href={`/admin/projects/${item.id}/edit`} className="rounded p-1 text-slate-400 hover:text-blue-600">
