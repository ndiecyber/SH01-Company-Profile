...
type Service = { id: string; icon: string; title: string };
...
export default function ServicesListPage() {
    ...
    useEffect(() => {
        async function load() {
            const { data } = await api.get<Service[]>("/cms/services");
            setItems(data);
            setLoading(false);
        }
        load();
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/admin/services/new" className="inline-flex items-center gap-1.5 rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand/90">
