...
type Reason = { id: string; icon: string; title: string };

export default function ReasonsListPage() {
    ...
    useEffect(() => {
        async function load() {
            const { data } = await api.get<Reason[]>("/cms/reasons");
            setItems(data);
            setLoading(false);
        }
        load();
    ...
    async function handleDelete(id: string) {
        try {
            await apiDelete("/cms/reasons/" + id);
            setRefresh((n) => n + 1);
        } catch (error) {
