...
type Technology = { id: string; icon: string; label: string; color: string };
...
export default function TechnologiesListPage() {
    ...
    useEffect(() => {
        async function load() {
            const { data } = await api.get<Technology[]>("/cms/technologies");
            setItems(data);
            setLoading(false);
        }
        load();
    ...
    async function handleDelete(id: string) {
        try {
            await apiDelete("/cms/technologies/" + id);
            setRefresh((n) => n + 1);
        } catch (error) {
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/admin/technologies/new" className="inline-flex items-center gap-1.5 rounded-md bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand/90">
                ...
                <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-left">
                        <tr>
                            ...
                            <th className="px-4 py-3 font-medium text-slate-500">Color</th>
                            ...
                            <tr key={item.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-2">
                                        <Link href={`/admin/technologies/${item.id}/edit`} className="rounded p-1 text-slate-400 hover:text-blue-600">
